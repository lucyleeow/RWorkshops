...
```{r}
peds %>%
  mutate(Season = ifelse(Month %in% c("January","February"), "Summer", ifelse(Month %in% c("March", "April", "May", "June", "July"), "Autumn", "Winter"))) %>%
  group_by(Season) %>%
  summarise(maen = mean(Hourly_Counts)) %>%
  ggplot() + 
  geom_bar(aes(y = maen, x = Season), stat = "identity")
```
